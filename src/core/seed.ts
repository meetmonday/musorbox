...
export async function seedCategories(): Promise<Record<string, number>> {
  ...
  const seededCategories = await db
    ...
    .from(categories);
  return Object.fromEntries(seededCategories.map((c) => [c.slug, c.id]));
}

export async function seedTags(): Promise<Record<string, number>> {
  ...
  return Object.fromEntries(seededTags.map((t) => [t.slug, t.id]));
}
